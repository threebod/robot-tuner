...
的发送仅执行急停。终端的 `stop`、`X/x`、`disable`、`disable5` 会取消待授权动作；
等待授权期间拒绝其他手动命令。固件返回 `ERR` 时，临时调试页和终端都会显示错误。

文本模式下“场地定位”页切换为安全航点导航：先选择实际摆放的启停区初始化，再点击地图选择最近安全航点，最后点击“移动到目标”。当前位置和航向来自固件的命令积分估计，不是真实定位；页面会用独立标记区分当前点与目标点。停止、急停、断连或固件故障后必须把车放回启停区并重新初始化。标准调试协议下该页面仍保持原有位姿同步行为。
