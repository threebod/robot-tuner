...
不超过 23 字节的 ASCII 命令，并补齐 CRLF；HEX 发送也不能绕过该限制。任意包含 `!`
...
临时调试页不再提供独立手动使能区，各运动按钮执行单次 `arm` 握手后由固件自动使能对应电机。普通停止按钮和全局急停仍保留。定距按100 mm步进，底盘测试转速上限为120 RPM；辅助电机可选择CAN地址5或6进行短点动。舵机2～4使用角度滑块，松开滑块时发送目标角度。
